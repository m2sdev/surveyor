...
jQuery(document).ready(function(){
  ...
  jQuery("#dependents").remove();
    ...
    function successfulSave(responseText){ // for(key in responseText) { console.log("key is "+[key]+", value is "+responseText[key]); }
      ...
      jQuery.each(responseText.hide, function(){ jQuery('#' + this).hide("fast"); clearInput(this); });
      ...
      jQuery.each(responseText.errors, function(k,v) {
        var element = jQuery('#q_'+k);
        var wrapper = element; //= element.closest('fieldset');
        var message = v; //todo: v is really an array of strings
        var errorElement = jQuery('<p class="inline_errors">' + message + '</p>'); // is this the template we want?

          wrapper.addClass('field_with_errors');
        wrapper.append(errorElement); // where does message go? at end, or after legend, or after input...?
        return false;
      });
    };

    // Used to clear old responses
    function clearInput(id) {
        //alert('//fieldset[@id="'+id+'"]/ol/input');
        var inputs = document.evaluate('//fieldset[@id="'+id+'"]/ol/li//input',document,null,XPathResult.ANY_TYPE,null);
        var inputFields = inputs.iterateNext();
        while (inputFields) {
          clearField(inputFields);
          inputFields = inputs.iterateNext();
        }
    }
